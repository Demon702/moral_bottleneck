...
def evaluate_file(file_path, model):
...
if __name__ == '__main__':
    ...
    model = args.model
    file_paths = glob.glob(f'../results/{model}/*.tsv')
    ...
    for file_name in FILE_NAMES:
        end_to_end_df = evaluate_file(f'../results/{model}/{file_name}.tsv', model)
        if file_name == 'end_to_end':
            GPT_DF_END_TO_END = end_to_end_df
