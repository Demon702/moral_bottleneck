...
class CoTFrame:
    def __init__(self, circumstance: str, gen: Dict[str, str]):
        ...
        for aspect_info in self.aspect_scores:
...
What will the person answer to the following questions?
...
Q2. Based on the answers to Q1, on a scale of 1 to 7, how morally acceptable is this scenario (1 means extremely immoral, 7 means highly moral)?
...
Your output format should be:
{
    "Answer to Q1": [
        {
            "aspect": "...aspect1",
            "demographic used to explain person's choice and answer": ["D1", "..", ...],
...
"""
    ...
    def __call__(self, scenario: str, circumstance: str, circumstance_type: str, demographic_info: pd.Series, human_score: Union[int, float], max_tokens: int = 1024, temperature: float = 0.0) -> CoTFrameWrapper:
        ...
        return CoTFrameWrapper(scenario=scenario, circumstance=circumstance, circumstance_type=circumstance_type, generation_json=generation_json, human_score=human_score)
        
       
def do_inference(maker: CoTFrameMaker, items: List[Tuple[str, str]], temperature: float = 0.0) -> List[CoTFrameWrapper]:
    outputs = []
    for item in tqdm(items):
        ...
        try:
            ...
            outputs.append(CoTFrameWrapper(scenario=scenario, circumstance=circumstance, circumstance_type=circumstance_type, human_score=human_score, generation_json={}))
    return outputs
...
def save_to_file(outpath_jsonl, inputs: Tuple[str, str], outputs: List[CoTFrameWrapper], append=False):
    sep="\t"
    columns = ['scenario_id', 'participant_id', 'dataset', 'scenario', 'circumstance_type', 'circumstance']
    for i in range(6):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--all_circumstances', action='store_true', default=False)
    ...
    openai_wrapper = OpenAIWrapper(cache_path=f'''demographic/cache/{model_dir_name}/cot.jsonl''')
    ...
    save_to_file(outpath_jsonl=f'''demographic/results/{model_dir_name}/cot{'_all_circumstances' if args.all_circumstances else ''}.tsv''', inputs=items, outputs=cfs)
