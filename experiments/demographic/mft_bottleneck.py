...
Create a frame for the given scenario. Your output format should be:
{
    "Answer to Q1": [
        {
            "norm name": "Harm/ Help",
            "demographic used to explain person's answer": ["D1", "..", ...],
...
"""
    ...
    def __call__(self, scenario: str, circumstance: str, circumstance_type: str, demographic_info: pd.Series, human_score: Union[int, float], max_tokens: int = 1024, temperature: float = 0.0) -> MFTFrameWrapper:
        ...
        return MFTFrameWrapper(scenario=scenario, circumstance=circumstance, circumstance_type=circumstance_type, generation_json=generation_json, human_score=human_score)
        
       
def do_inference(maker: MFTFrameMaker, items: List[Tuple[str, str]], temperature: float = 0.0) -> List[MFTFrameWrapper]:
    outputs = []
    for item in tqdm(items):
        ...
        try:
            ...
            outputs.append(MFTFrameWrapper(scenario=scenario, circumstance=circumstance, circumstance_type=circumstance_type, human_score=human_score, generation_json={}))
    return outputs
...
if __name__ == '__main__':
    ...
    openai_wrapper = OpenAIWrapper(cache_path=f'''demographic/cache/{model_dir_name}/mft.jsonl''')
    ...
    save_to_file(outpath_jsonl=f'''demographic/results/{model_dir_name}/mft{'_all_circumstances' if args.all_circumstances else ''}.tsv''', inputs=items, outputs=cfs)
