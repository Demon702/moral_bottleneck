...
class MoralTheoryFrameWrapper:
    def __init__(self, scenario: str, prompt: str, generation_json: str, circumstance: str = '', circumstance_type: str = 'orig', human_score: float = 0.0, num_addn_qns: int = 0):
...
class MoralTheoryFrameMaker:
    ...
    def __init__(self, engine: str, client = None, openai_wrapper = None, theory_prompt: str = '', num_addn_qns: int = 0):
        super().__init__()
        self.client = client
        self.engine = engine
    ...
    def __call__(self, scenario: str, circumstance: str, circumstance_type: str, human_score: float, max_tokens: int= 512) -> MoralTheoryFrameWrapper:
        ...
        return MoralTheoryFrameWrapper(scenario=scenario, circumstance=circumstance, circumstance_type=circumstance_type, prompt=generation_query, generation_json=generation_json, human_score=human_score, num_addn_qns=self.num_addn_qns)


def do_inference(maker: MoralTheoryFrameMaker, items: List[Tuple[str, str]]) -> List[MoralTheoryFrameWrapper]:
    outputs = []
    for item in tqdm(items):
        ...
        try:
            ...
            outputs.append(MoralTheoryFrameWrapper(scenario=scenario, circumstance=circumstance, circumstance_type=circumstance_type, prompt=prompt or f"Failed prompt with exception {exc}", generation_json={}))
    return outputs
...
if __name__ == '__main__':
    ...
    MODEL_CONFIG = {
        ...
        "meta/llama3-70b-instruct": "nvdev/meta/llama3-70b-instruct",
        "meta/llama3-8b-instruct": "meta-llama/Llama-3-8b-chat-hf",
        "mistralai/mixtral-8x22b-instruct-v0.1": "nvdev/mistralai/mixtral-8x22b-instruct-v0.1",
    ...
    maker = MoralTheoryFrameMaker(engine=MODEL_CONFIG.get(args.model, args.model), openai_wrapper=openai_wrapper, client=client, theory_prompt=theory_prompt, num_addn_qns=num_addn_qns)
