...
class DyadicFrameMaker:
    ...
    def __init__(self, engine: str, client: OpenAI, verbose: bool = False):
        super().__init__()
        self.client = client
        self.engine = engine
...
Create a frame for the given scenario. The frame contains an answer for each question and a very short reasoning for Q5-Q9.

Let's think step by step before producing the frame.
...
"""
    ...
    def post_process_generation(self, generated_str: str) -> Tuple[str, str]:
        # Capture <thinking>...</thinking> block before stripping (handles both prompted CoT and native thinking models)
        think_match = re.search(r"<thinking>(.*?)</thinking>", generated_str, flags=re.DOTALL)
        think_trace = think_match.group(1).strip() if think_match else ""

        generated_str = re.sub(r"<thinking>(.*?)</thinking>", "", generated_str, flags=re.DOTALL)
...
if __name__ == '__main__':
    ...
    maker = DyadicFrameMaker(engine=engine, client=client, verbose=args.verbose)
    ...
    save_to_file(outpath_jsonl=f'''results/{model_dir_name}/cot_dyadic{'_all_circumstances' if args.all_circumstances else ''}.tsv''', inputs=items, outputs=cfs)
